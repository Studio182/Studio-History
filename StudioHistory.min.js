...
function StudioHistory(){this.history=new Array;this.history_redo=new Array;this.history_location=-1;this.history_redo_location=-1;this.history_val="";this.history_redo_offset=0;Array.prototype.remove=function(from,to){var rest=this.slice((to||from)+1||this.length);this.length=from<0?this.length+from:from;return this.push.apply(this,rest)};this.difflib=new diff_match_patch();this.setHistoryItem=function(current){var diff=this.difflib.patch_make(current,this.history_val);this.history_val=current;this.history.push(diff);this.history_location-=this.history_redo_offset;if(this.history.length-1>Math.floor(this.history_location+1)){var remove=(this.history.length-1)-Math.floor(this.history_location+1);this.history.splice(this.history.length-remove,remove)}this.history_location++;this.history_redo_location=-1;this.history_redo_offset=0;this.history_redo=new Array;console.log(this)};this.getUndoItem=function(){if(this.history_location-1!=-1){var value=this.difflib.patch_apply(this.history[this.history_location],this.history_val);this.history_redo.push(this.difflib.patch_make(value[0],this.history_val));this.history_redo_location++;this.history_offset++;this.history_val=value[0];this.history_location--;return value[0]}else{return false}};this.getRedoItem=function(){var count=this.history_redo.length-1;if(this.history_redo[count]!=null){var value=this.difflib.patch_apply(this.history_redo[count],this.history_val);this.history_redo.remove(count);this.history_val=value[0];this.history_location++;this.history_offset++;return value[0]}else{return false}}this.exportData=function(){var edata={};edata['history']=this.history;edata['history_location']=this.history_location;edata['history_redo']=this.history_redo;edata['history_redo_location']=this.history_redo_location;edata['history_redo_offset']=this.history_redo_offset;edata['history_val']=this.history_val;return JSON.stringify(edata)}this.importData=function(data){var idata=JSON.parse(data)this.history=idata['history'];this.history_location=idata['history_location'];this.history_redo=idata['history_redo'];this.history_redo_location=idata['history_redo_location'];this.history_redo_offset=idata['history_redo_offset'];this.history_val=idata['history_val']}return true};
...
function diff_match_patch(){this.Diff_Timeout=1.0;this.Diff_EditCost=4;this.Match_Threshold=0.5;this.Match_Distance=1000;this.Patch_DeleteThreshold=0.5;this.Patch_Margin=4;this.Match_MaxBits=32;}
